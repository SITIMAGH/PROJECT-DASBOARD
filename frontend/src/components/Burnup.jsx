...
function BurndownChart() {
  const [selectedView, setSelectedView] = useState("BurnUp", "BurnDown");//pertama kali di pilih
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.log("cumulativeAddedTotal:", cumulativeAddedTotal);
        ...
        if (selectedYearData && selectedYearData.progress) {
          ...
          for (let i = 1; i < months.length; i++) {
            //kalo mau nyambung gambar nya 0 nya jadiin ke 1
            if (cumulativeDoneData[i] === 0) {
              cumulativeDoneData[i] = cumulativeDoneData[i - 1]; // Gunakan nilai kumulatif bulan sebelumnya
            }
            if (cumulativeAddedData[i] === 0) {
              cumulativeAddedData[i] = cumulativeAddedData[i - 1]; // Gunakan nilai kumulatif bulan sebelumnya
            }
          }
          ...
          console.log("remainwork:", remainingWorkData);
