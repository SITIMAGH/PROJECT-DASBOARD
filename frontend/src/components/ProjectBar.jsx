  ...
  const fetchLineChartData = async (selectedTypeValues, selectedVersionValue) => {
    try {
      ...
      if (filteredProjects.length > 0) {
        const cumulativeDoneData = [0];
        const cumulativeAddedData = [0];
        const lineData = {
            ...
            if (selectedVersionData) {
              for (const version of selectedVersionData.versions) {
                if (version.version_name === selectedVersionValue) {
                  for (let i = 1; i < version.progress.length; i++) {
                    lineData.labels.push(version.progress[i].date);


                    //komulatif dobe
                    const  cumulativeDoneData = lineData.datasets[0].data[lineData.datasets[0].data.length-1]+ entry.wp_done;
                    lineData.datasets[0].data.push(cumulativeDoneData);

                    //komulatif on going
                    const  cumulativeAddedData = lineData.datasets[1].data[lineData.datasets[0].data.length-1]+ entry.wp_on_going;
                    lineData.datasets[1].data.push(cumulativeAddedData);
